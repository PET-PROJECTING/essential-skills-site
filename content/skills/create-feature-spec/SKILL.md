...
description: >-
  ...
  feature, /create-feature-spec, or to spec the next item in the build planner.
...
Turn one build-planner unit into a spec that a later session can implement. **Do not implement application code.** **Do not mark units Completed** in the progress tracker.
...
3. If that section is missing, search the repo for `progress-tracker.md` (skip `node_modules`, `.git`, dist/build). If several match, ask which one. The spec folder is the sibling directory that contains `build-planner.md` (or the older `00-build-plan.md`), or `feature-specs/` under the same root.
...
- If `project-overview.md` is still unfilled, grill and write **only that file** (specify-context Phase 3), then **stop**. Tell the user to re-run `/specify-context` until the six files and build planner exist, then `/create-feature-spec` again.
...
Read, in order: project-overview, architecture, ui-context, code-standards, ai-workflow-rules, progress-tracker, and `build-planner.md` if it exists. If only the older `00-build-plan.md` is present, treat that file as the build planner.

## Phase 2 — Choose the unit

If `build-planner.md` exists (even if still a template), grill in one round:
...
If the build planner is still unfilled, treat this as C after a short grill for the first unit, and write a real first row into `build-planner.md`. Do not invent a full multi-unit plan here — suggest `/specify-context` to fill the rest of the plan.
...
2. Update `build-planner.md`: set that row's Spec column to the filename.
...
| `/specify-context` | Context or build planner needs a revision |
